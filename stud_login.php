...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($captcha_input) || strtoupper($captcha_input) !== $_SESSION['captcha']) {
        ...
        if ($conn->connect_error) {
            ...
            // Validate username + password from students table
            $stmt = $conn->prepare("SELECT password, created_at FROM students WHERE username = ?");
            ...
            if ($stmt->num_rows > 0) {
                ...
                if (password_verify($password, $hashed_password)) {
                    ...
                    $_SESSION['login_time'] = $created_at;
                    unset($_SESSION['captcha']);
                    header("Location: stud_result.php"); // Redirect to student result page
                    exit;
                } else {
...
<header>
<h1></h1>
  <h4>Dy Patil Intstiute Of Engineering Management And Resarch</h4>    <h2>Student Login</h2>
</header>
<div class="container">
    <div class="form-container">
        ...
        <p style="margin-top: 15px;">Don't have an account? <a href="stud_sign.php">Sign up now</a></p>
    </div>
</div>
</body>
</html>
